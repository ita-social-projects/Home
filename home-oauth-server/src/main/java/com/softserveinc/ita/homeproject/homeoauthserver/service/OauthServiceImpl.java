...
public class OauthServiceImpl implements OauthService {
    ...
    public Token generateToken(UserDetailsDto userDetailsDto) {
        UserCredentials userCredentials = findByEmailAndPassword(userDetailsDto.getEmail(), userDetailsDto.getPassword());
    ...
    private UserCredentials findByEmailAndPassword(String email, String password) {
        ...
        UserCredentials userCredentials = userCredentialsRepository.findByEmail(email).filter(UserCredentials::getEnabled)
            .orElseThrow(() -> new NotFoundOauthException(USER_NOT_FOUND_FORMAT));
